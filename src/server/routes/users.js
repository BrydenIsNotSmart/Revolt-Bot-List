...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!fetcheduser) {
      ...
      return res.render("error.ejs", {
        ...
        code: 404,
        message:
          "This user could not be found on Revolt, might be due to the bot's cache.",
      });
    }
  } catch (err) {
  ...
  if (!fetched_user)
    fetched_user = await userModel.create({
      revoltId: id,
      verified: false,
      createdAt: Date.now(),
    });
  if (fetched_user) {
    try {
      ...
      if (!fetched_user.bio)
        fetched_user.bio = "This user does not have a bio.";
    } catch (err) {
...
router.get("/:id/edit", checkAuth, async (req, res) => {
  if (!req.params.id === req.session.userAccountId)
    return res
      .status(403)
...
router.post("/:id/edit", checkAuth, async (req, res) => {
  ...
  const data = req.body;
  if (!data)
    return res.status(400).json("No data was changed within your profile!");
  ...
  await user.save().then(async () => {
    res
      .status(201)
      .redirect(`/users/${req.session.userAccountId}?message=Success`);
