...
router.get(
  "/reports/:reporterId/:userId/:botId/:type/resolve",
  async (req, res) => {
    let user = await userModel.findOne({ revoltId: req.session.userAccountId });
    if (user) {
      let userRaw = await client.users.fetch(user?.revoltId);
    ...
    await report
      ...
      .then(
        res
          .status(200)
          .redirect(
            "/panel/reports?success=true?message=You have successfully resolved the report."
          )
      );
  }
});
...
router.post("/bots/:id/testing", async (req, res) => {
  ...
  if (bot.status === "approved")
    return res.status(400).json({ message: "This bot is already approved." });
  if (bot.status === "denied")
    return res.status(400).json({ message: "This bot is already denied." });
  if (bot.status === "inprogress")
    return res
      .status(400)
      .json({ message: "This bot is already being tested." });
  ...
  let cat = [];
  server.categories.map((d) => {
    if (d.id === "01GX1R43Q03JF7SMBKRW71F7EF")
      d.channels.push(createdChannel._id);
    cat.push(d);
  });
...
router.get("/bots/:id/approve", async (req, res) => {
  ...
  if (bot.status === "approved" || bot.status === "denied")
    return res
      .status(400)
      .json({ message: "This bot has already been approved or denied." });
...
router.get("/bots/:id/deny", async (req, res) => {
  ...
  if (!bot || bot.deleted)
    return res
      .status(404)
      .json({ message: "This bot was not found or it has been deleted" });
  if (bot.status === "approved" || bot.status === "denied")
    return res
      .status(400)
      .json({ message: "This bot has already been approved or denied." });
...
router.post("/bots/:id/deny", async (req, res) => {
  ...
  await bot.save().then(async () => {
    ...
    let testingChannel = testing.channels.find(
      (c) => c.name === `${bot.name.toLowerCase()}`
    );
    try {
        ...
        await user
          ...
          .then((dm) => {
            dm.sendMessage(
              `:x: Your bot **${
                bot.name
              }** has been **denied** on RevoltBots.org!\n**Reviewer**: <@${
                bot.reviewer
  ...
  await bot.save().then(async () => {
    ...
    try {
      ...
      bot.owners.forEach(async (owner) => {
        ...
        let user = await client.users.fetch(owner).catch(() => {});
        await client.api
          .get(`/servers/${config.servers.main}/members/${owner}`)
          .then(async (data) => {
            if (data.roles) data.roles.map((e) => roles.push(e));
            roles.push(config.roles.developer);
            await sleep(700);
            await client.api
              .patch(`/servers/${config.servers.main}/members/${owner}`, {
                roles: roles,
              })
              .catch(() => {
                return res
                  .status(400)
                  .json({ message: `Unable to add role to this user.` });
              });
          });
        await user
          ?.openDM()
          .then((dm) => {
            dm.sendMessage(
              `:white_check_mark: Your bot **${
                bot.name
              }** has been **approved** on RevoltBots.org!\n**Reviewer**: <@${
                bot.reviewer
              }>\n**Feedback**: ${req.body.feedback || "None provided."}`
            );
          })
          .catch(() => {
            return;
...
router.post("/badges/add", async (req, res) => {
  ...
  if (!user)
    return res
      .status(404)
      .json({ message: "User was not found within database." });
  ...
  if (req.body.badge === "staff") {
    ...
    client.api.get(
      `/servers/${config.servers.main}/members/${req.body.userId}`
    );
    await client.api
      .get(`/servers/${config.servers.main}/members/${req.body.userId}`)
      .then(async (data) => {
        if (data.roles) data.roles.map((e) => roles.push(e));
        roles.push(config.roles.staff);
        await sleep(700);
        await client.api
          .patch(`/servers/${config.servers.main}/members/${req.body.userId}`, {
            roles: roles,
          })
          .catch(() => {
            return res
              .status(400)
              .json({
                message: `Unable to add role to this user but I added them as a ${req.body.badge}.`,
              });
          });
      });
  } else if (req.body.badge === "partner") {
    if (user.badges.includes("partner"))
    ...
    if (user.badges.includes("contributor"))
      return res
        .status(400)
        .json({ message: "User is already a contributor." });
  ...
  user.save().then(() => {
    res
      .status(201)
      .json({ message: `Successfully added badge ${req.body.badge} to user.` });
  });
});
...
router.post("/badges/remove", async (req, res) => {
  ...
  if (req.body.badge === "staff") {
    ...
    await client.api
      ...
      .then(async (res) => {
        res.roles
          .filter((e) => e != config.roles.partner)
          .map((e) => roles.push(e));
        await sleep(700);
        await client.api
          .patch(`/servers/${config.servers.main}/members/${req.body.userId}`, {
    ...
    await client.api
      ...
      .then(async (res) => {
        res.roles
          .filter((e) => e != config.roles.contributor)
          .map((e) => roles.push(e));
        await sleep(700);
        await client.api
          .patch(`/servers/${config.servers.main}/members/${req.body.userId}`, {
  ...
  user.save().then(() => {
    res
      .status(201)
      .json({
        message: `Successfully removed badge ${req.body.badge} to user.`,
      });
  });
});
...
router.get("/certification/:id/approve", async (req, res) => {
  ...
  if (!bot || bot.deleted)
    return res
      .status(404)
      .json({ message: "This bot could not be found on our list." });
  if (!bot.certifyApplied)
    return res
      .status(400)
      .json({ message: "This bot has already been denied for certification." });
...
router.post("/certification/:id/deny", async (req, res) => {
  ...
  await bot.save().then(async () => {
    try {
      res
        .status(201)
        .json({ message: "Successfully Denied Certification", code: "OK" });
      let logs = client.channels.get(config.channels.weblogs);
      logs.sendMessage(
        `<\\@${bot.owners[0]}>'s bot **${
          bot.name
        }** has been **denied** for **certification** by <\\@${
          req.session.userAccountId
        }>.\n<https://revoltbots.org/bots/${bot.id}>\n**Reason**: ${
          req.body.reason || "None provided."
        }`
        ...
        await user
          ...
          .then((dm) => {
            dm.sendMessage(
              `:x: Your bot **${
                bot.name
              }** has been **denied** for **certification** on RevoltBots.org!\n**Reviewer**: <@${
                req.session.userAccountId
              }>\n**Reason**: ${req.body.reason || "None provided."}`
            );
          })
...
router.post("/certification/:id/approve", async (req, res) => {
  ...
  await bot.save().then(async () => {
    try {
      res
        .status(201)
        .json({ message: "Successfully Approved Certification", code: "OK" });
      let logs = client.channels.get(config.channels.weblogs);
      logs.sendMessage(
        ...
        await user
          ...
          .then((dm) => {
            dm.sendMessage(
              `:white_check_mark: Your bot **${
                bot.name
              }** has been **certified** on RevoltBots.org!\n**Reviewer**: <@${
                req.session.userAccountId
              }>\n**Feedback**: ${req.body.feedback || "None provided."}`
            );
          })
      ...
      await client.api
        .get(`/servers/${config.servers.main}/members/${req.params.id}`)
        .then(async (data) => {
          const roles2 = [];
          ...
          await sleep(700);
          await client.api
            .patch(`/servers/${config.servers.main}/members/${req.params.id}`, {
              roles: roles2,
            })
            .catch(() => {
              res
                .status(400)
                .json({
                  message: `Unable to add role to this user but I certified the user and bot. (p2)`,
                });
            });
        });
    } catch (err) {
...
router.get("/certification/:id/certifyDelete", checkAdmin, async (req, res) => {
  ...
  if (!bot.certified)
    return res
      .status(400)
      .json({ message: "This bot has isn't certification." });
...
router.post(
  "/certification/:id/certifyDelete",
  checkAdmin,
  async (req, res) => {
    let bot = await botModel.findOne({ id: req.params.id });
    if (!bot || bot?.deleted)
    ...
    await bot.save().then(async () => {
      try {
        res
          .status(201)
          .json({ message: "Successfully Deleted Certification", code: "OK" });
        let logs = client.channels.get(config.channels.weblogs);
        logs.sendMessage(
          `<\\@${bot.owners[0]}>'s bot **${
            bot.name
          }** **certification** was **deleted**  by <\\@${
            req.session.userAccountId
          }>.\n<https://revoltbots.org/bots/${bot.id}>\n**Reason**: ${
            req.body.reason || "None provided."
          }`
          ...
          await user
            ...
            .then((dm) => {
              dm.sendMessage(
                `:x: Your bot **${
                  bot.name
                }**'s **certification** was **deleted** on RevoltBots.org!\n**Reviewer**: <@${
                  req.session.userAccountId
                }>\n**Reason**: ${req.body.reason || "None provided."}`
              );
            })
