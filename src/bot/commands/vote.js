...
module.exports = {
    ...
    async run(client, message, args) {
      try {
        ...
        BotRaw = await client.users.fetch(args[0].match(new RegExp(`(<@!?(.*)>)`))[2])
        } else { 
        BotRaw = await client.users.fetch(args[0]).catch(() => { BotRaw = false; })
        } 
        if (!BotRaw || !BotRaw.bot) return message.reply("This is not a real bot. :hmm:");

        let bot = await botModel.findOne({ id: BotRaw._id });
        if (!bot)
        return message.reply("This bot was not found on our list.")
        ...
        if (x) {
          ...
          if (!vote.status)
            return message.reply(`Please wait ${vote.formatted} before you can vote again.`)
